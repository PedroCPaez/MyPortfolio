...
def write_new_task_to_tasks_file(task_list):
    ...
    with open("tasks.txt", "w+") as task_file:
        for i, task in enumerate(task_list, start=1):
            task_data = [
                str(i),  # Número de tarea consecutivo
                task['username'],
                task['title'],
                task['description'],
            ...
            print(f"Task {i} successfully added to output file tasks.txt")
...
def main():
    ...
    task_data = read_tasks_file()  # Llamada a la función para obtener los datos de las tareas
    print(task_data)
    task_list = create_task_list(task_data)
    print("TRES")
    print(task_list)
    ...
    while True:
        ...
        while True:
            ...
            if user_option == "r":
                ...
                # Llamada a registration_user_option() con el argumento username_password
            ...
            if user_option == "vm":
                ...
                if specific_task == "-1":
                    display_main_menu()
"""                else:
                    specific_task = str(specific_task)  # Convertir a cadena
                    if validate_if_task_number_in_my_task_list(specific_task, my_task_list):
                        if validate_my_task_status(specific_task, my_task_list):
                            # Realizar acciones según la decisión del usuario
                            display_edit_or_complete()
                            user_option = input("Select an option: ").lower()
                            validate_data_entry(user_option)
                            #change et to eu
                            if user_option == "eu":
                                edit_task(task_list, specific_task)  # Llamar a la función para editar la tarea
                            #change ct to edd
                            elif user_option == "ct":
                                complete_task(task_list, specific_task)  # Llamar a la función para completar la tarea
                        else:
                            print("Task already completed. Cannot be edited.")
                    else:
                        print("Invalid task number.")

                write_task_list_to_file(task_list)  # Actualizar el archivo tasks.txt
                write_task_list_to_file(task_list)  # Actualizar el archivo tasks.txt
